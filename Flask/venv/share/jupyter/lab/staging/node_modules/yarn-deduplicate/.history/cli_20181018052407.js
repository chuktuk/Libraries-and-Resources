...
const fixDuplicates = require('./index');
...
commander
    ...
    .option('-n, --dry-run', 'Do not change yarn.lock, just output the diagnosis')
    .option('--packages <packages>', 'A comma separated list of packages to dedupe. Defauls to all packages.', val => val.split(',').map(v => v.trim()))
    .option('--stdout', 'Instead of changing yarn.lock, output the new deduped yarn.lock to stdout')
