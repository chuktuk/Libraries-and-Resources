...
module.exports.listDuplicates = (yarnLock, {
    ...
    Object.keys(packages)
        ...
        .forEach(packageInstance => {
            const bestVersion = packageInstance.bestVersion;
            if (bestVersion !== packageInstance.pkg.version) {
                result.push(`Package "${packageInstance.name}" wants ${packageInstance.requestedVersion} and could get ${bestVersion}, but got ${packageInstance.pkg.version}`);
            }
        });
...
module.exports.fixDuplicates = (yarnLock, {
    ...
    Object.keys(packages)
        .reduce((acc, name) => acc.concat(getPackageIntances(packages, name, useMostCommon)), [])
        .forEach(packageInstance => {
            const {bestVersion, name, requestedVersion, versions, pkg} = packageInstance.bestVersion;
            if (bestVersion !== pkg.version) {
